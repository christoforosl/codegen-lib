...
namespace CsModelMappers {
	...
	public class EmployeeProjectDBMapper : DBMapper {
		...
		public override IDbCommand getUpdateDBCommand(IModelObject modelObj, string sql) {
			...
			stmt.Parameters.Add( this.dbConn.getParameter("@EPEmployeeId",obj.PrEPEmployeeId));
			stmt.Parameters.Add( this.dbConn.getParameter("@EPProjectId",obj.PrEPProjectId));
			stmt.Parameters.Add( this.dbConn.getParameter("@AssignDate",obj.PrAssignDate));
			stmt.Parameters.Add( this.dbConn.getParameter("@EndDate",obj.PrEndDate));
			stmt.Parameters.Add( this.dbConn.getParameter("@Rate",obj.PrRate));

			if ( obj.isNew ) {
			} else {
			//only add primary key if we are updating and as the last parameter
							stmt.Parameters.Add( this.dbConn.getParameter("@EmployeeProjectId",obj.PrEmployeeProjectId));
		}
	...
	public class EmployeeProjectDataReaderLoader : DataReaderLoader {
		public override void load(IModelObject mo) {
			...
			if (!this.reader.IsDBNull(DATAREADER_FLD_EMPLOYEEPROJECTID) ) {
				obj.PrEmployeeProjectId = this.reader.GetInt32(DATAREADER_FLD_EMPLOYEEPROJECTID);
			}
			if (!this.reader.IsDBNull(DATAREADER_FLD_EPEMPLOYEEID) ) {
				obj.PrEPEmployeeId = this.reader.GetInt32(DATAREADER_FLD_EPEMPLOYEEID);
			}
			if (!this.reader.IsDBNull(DATAREADER_FLD_EPPROJECTID) ) {
				obj.PrEPProjectId = this.reader.GetInt32(DATAREADER_FLD_EPPROJECTID);
			}
