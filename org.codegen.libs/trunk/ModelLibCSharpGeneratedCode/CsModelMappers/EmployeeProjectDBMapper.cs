...
//************************************************************
// 
// Class autogenerated on13-04-2015 22:29:26 by ModelGenerator
...
namespace CsModelMappers {
	...
	public class EmployeeProjectDBMapper : DBMapper {
		...
		public override IDbCommand getUpdateDBCommand(IModelObject modelObj, string sql) {
			IEmployeeProject obj = (IEmployeeProject)modelObj;
			IDbCommand stmt = this.dbConn.getCommand(sql);
			stmt.Parameters.Add( this.dbConn.getParameter("@EPEmployeeId",obj.EPEmployeeId));
			stmt.Parameters.Add( this.dbConn.getParameter("@EPProjectId",obj.EPProjectId));
			stmt.Parameters.Add( this.dbConn.getParameter("@AssignDate",obj.AssignDate));
			stmt.Parameters.Add( this.dbConn.getParameter("@EndDate",obj.EndDate));
			stmt.Parameters.Add( this.dbConn.getParameter("@Rate",obj.Rate));

			if ( obj.isNew ) {
			} else {
			//only add primary key if we are updating and as the last parameter
							stmt.Parameters.Add( this.dbConn.getParameter("@EmployeeProjectId",obj.EmployeeProjectId));
		}
	...
	public override void saveParents(IModelObject mo ){
		...
		if ((thisMo.Project!=null) && (thisMo.Project.NeedsSave)) {
			CsModelMappers.ProjectDBMapper mappervar = new CsModelMappers.ProjectDBMapper(this.dbConn);
			mappervar.save(thisMo.Project);
			thisMo.EPProjectId = thisMo.Project.ProjectId;
		}
	...
	public class EmployeeProjectDataReaderLoader : DataReaderLoader {
		public override void load(IModelObject mo) {
			...
			if (!this.reader.IsDBNull(DATAREADER_FLD_EMPLOYEEPROJECTID) ) {
				obj.EmployeeProjectId = this.reader.GetInt32(DATAREADER_FLD_EMPLOYEEPROJECTID);
			}
			if (!this.reader.IsDBNull(DATAREADER_FLD_EPEMPLOYEEID) ) {
				obj.EPEmployeeId = this.reader.GetInt32(DATAREADER_FLD_EPEMPLOYEEID);
			}
			if (!this.reader.IsDBNull(DATAREADER_FLD_EPPROJECTID) ) {
				obj.EPProjectId = this.reader.GetInt32(DATAREADER_FLD_EPPROJECTID);
			}
			if (!this.reader.IsDBNull(DATAREADER_FLD_ASSIGNDATE) ) {
				obj.AssignDate = this.reader.GetDateTime(DATAREADER_FLD_ASSIGNDATE);
			}
			if (!this.reader.IsDBNull(DATAREADER_FLD_ENDDATE) ) {
				obj.EndDate = this.reader.GetDateTime(DATAREADER_FLD_ENDDATE);
			}
			if (!this.reader.IsDBNull(DATAREADER_FLD_RATE) ) {
				obj.Rate = this.reader.GetDecimal(DATAREADER_FLD_RATE);
			}
	...
	public sealed class EmployeeProjectDataUtils {
		...
		public static EmployeeProject loadFromDataRow(DataRow r) {
			...
			IModelObject mo = EmployeeProjectFactory.Create();
			a.DataSource = r;
			a.load(mo);
