...
Namespace VbBusObjects.DBMappers
	<System.Runtime.InteropServices.ComVisible(False)><SelectObject("<SELECT_FROM_NAME>")><KeyFieldName("EmployeeInfoId")> _
...
#Region "getUpdateDBCommand"
        Public Overrides Function getUpdateDBCommand(ByVal modelObj As IModelObject, ByVal sql As String) As IDbCommand
			...
			stmt.Parameters.Add(Me.dbConn.getParameter(EmployeeInfo.STR_FLD_EIEMPLOYEEID,obj.PrEIEmployeeId))
			stmt.Parameters.Add(Me.dbConn.getParameter(EmployeeInfo.STR_FLD_SALARY,obj.PrSalary))
			stmt.Parameters.Add(Me.dbConn.getParameter(EmployeeInfo.STR_FLD_ADDRESS,obj.PrAddress))

			If (obj.isNew) Then
			Else
			'only add primary key if we are updating and as the last parameter
				stmt.Parameters.Add(Me.dbConn.getParameter(EmployeeInfo.STR_FLD_EMPLOYEEINFOID,obj.PrEmployeeInfoId))
			End if
	...
	Public Class EmployeeInfoDataReaderLoader
		Inherits DataReaderLoader
			...
			if me.reader.IsDBNull(DATAREADER_FLD_EMPLOYEEINFOID) = false Then
				obj.PrEmployeeInfoId = Convert.ToInt64(me.reader.GetInt32(DATAREADER_FLD_EMPLOYEEINFOID))
			End if
			if me.reader.IsDBNull(DATAREADER_FLD_EIEMPLOYEEID) = false Then
				obj.PrEIEmployeeId = Convert.ToInt64(me.reader.GetInt32(DATAREADER_FLD_EIEMPLOYEEID))
			End if
