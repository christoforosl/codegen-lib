...
#Region "Fields"
    ...
    Private _managedTableName As String
    Private _selectObject As String
    Private _pkFieldname As String

    ''' <summary>
    ''' The name of the database table that this mapper object selects, Inserts and Updates
    ''' </summary>
    Public Property isPrimaryKeyAutogenerated As Boolean
        Get
            Return _isPrimaryKeyAutogenerated
        End Get
        Private Set(value As Boolean)
            _isPrimaryKeyAutogenerated = value
        End Set
    End Property
    ...
    ''' The name of the database table that this mapper object selects, Inserts and Updates
    ''' </summary>
    Public Property ManagedTableName As String
        Get
            Return _managedTableName
        End Get
        Private Set(value As String)
            _managedTableName = value
        End Set
    End Property

    ''' <summary>
    ''' The name of the database table or view that this dbmapper selects data from
    ''' </summary>
    Public Property SelectFromObjectName As String
        Get
...
#Region "Constructors"
    ...
    Private Sub determineConf()
        ...
        If Not (attr Is Nothing) Then
            Me.isPrimaryKeyAutogenerated = attr.IsPrimaryKeyAutogenerated
        End If

        Dim sattrKeyFieldName As KeyFieldName = CType(Attribute.GetCustomAttribute(Me.GetType, GetType(KeyFieldName)), KeyFieldName)

        If sattrKeyFieldName Is Nothing Then
            Throw New ApplicationException("Attribute KeyFieldName is required but not found on this mapper")
        End If
        Me.pkFieldName = sattrKeyFieldName.KeyFieldName

        sattr = Attribute.GetCustomAttribute(Me.GetType, GetType(SelectObject))
        Dim attrSelectObject As SelectObject = CType(sattr, SelectObject)
        If Not (attrSelectObject Is Nothing) Then
            Me.SelectFromObjectName = attrSelectObject.SelectObjectName
        End If

        sattr = Attribute.GetCustomAttribute(Me.GetType, GetType(System.Data.Linq.Mapping.TableAttribute))
        Dim attrTname As System.Data.Linq.Mapping.TableAttribute = CType(sattr, System.Data.Linq.Mapping.TableAttribute)
        If Not (attrTname Is Nothing) Then
            Me.ManagedTableName = attrTname.Name
        End If
...
#End Region
    ...
    ''' The select count(*) is done on the managed table, NOT on the select object
    ''' </summary>
    Public Function RecordCount() As Long

        Return Me.dbConn.getLngValueWithParams("select count(*) from " & Me.ManagedTableName)
...
Public MustInherit Class OracleDBMapper
    Inherits DBMapper
    ...
    '''	 <summary>  Performs an <b>insert</b> operation to the database </summary>
    ''' <param name="mo"> ModelObject to save to database </param>
    Public Overrides Sub insert(ByVal mo As IModelObject)
        ...
        Try
            ...
            If Me.isPrimaryKeyAutogenerated Then
                ' set the ID field here
                mo.Id = CType(pstmt.Parameters.Item(pstmt.Parameters.Count - 1), IDataParameter).Value
            End If
