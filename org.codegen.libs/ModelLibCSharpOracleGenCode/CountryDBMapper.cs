...
//************************************************************
// 
// Class autogenerated on21-Apr-15 4:18:34 PM by ModelGenerator
...
[AttrIsPrimaryKeyAutogenerated(false)]
	public class CountryDBMapper : DBMapper {
		...
		public override IDbCommand getUpdateDBCommand(IModelObject modelObj, string sql) {
			ICountry obj = (ICountry)modelObj;
			IDbCommand stmt = this.dbConn.getCommand(sql);
				stmt.Parameters.Add(this.dbConn.getParameter("@COUNTRY_NAME",obj.PrCountryName));
				stmt.Parameters.Add(this.dbConn.getParameter("@REGION_ID",obj.PrRegionId));
				stmt.Parameters.Add(this.dbConn.getParameter("@SKIP_FIELD",obj.PrSkipField));
				stmt.Parameters.Add(this.dbConn.getParameter("@LONG_FLD",obj.PrLongFld));
				stmt.Parameters.Add(this.dbConn.getParameter("@LONG_FLD2",obj.PrLongFld2));
...
;
	...
	public class CountryDataReaderLoader : DataReaderLoader {
		public override void load(IModelObject mo) {
			...
			if (!this.reader.IsDBNull(DATAREADER_FLD_REGION_ID) ) {
				obj.PrRegionId = this.reader.GetDecimal(DATAREADER_FLD_REGION_ID);
			}
			...
			if (!this.reader.IsDBNull(DATAREADER_FLD_LONG_FLD) ) {
				obj.PrLongFld = this.reader.GetDecimal(DATAREADER_FLD_LONG_FLD);
			}
			if (!this.reader.IsDBNull(DATAREADER_FLD_LONG_FLD2) ) {
				obj.PrLongFld2 = this.reader.GetDecimal(DATAREADER_FLD_LONG_FLD2);
			}
	...
	public sealed class CountryDataUtils {
		...
		public static Country loadFromDataRow(DataRow r) {
			...
			IModelObject mo = CountryFactory.Create();
			a.DataSource = r;
			a.load(mo);
			return (Country)mo;
