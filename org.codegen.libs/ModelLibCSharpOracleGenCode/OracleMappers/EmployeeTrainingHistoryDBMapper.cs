...
//************************************************************
// 
// Class autogenerated on12-May-15 11:20:41 AM by ModelGenerator
...
namespace OracleMappers {
	...
	public class EmployeeTrainingHistoryDBMapper : DBMapper {
		...
		public override IDbCommand getUpdateDBCommand(IModelObject modelObj, string sql) {
			IEmployeeTrainingHistory obj = (IEmployeeTrainingHistory)modelObj;
			IDbCommand stmt = this.dbConn.getCommand(sql);
				stmt.Parameters.Add(this.dbConn.getParameter("@EMPLOYEE_ID",obj.PrEmployeeId));
				stmt.Parameters.Add(this.dbConn.getParameter("@DATE_FROM",obj.PrDateFrom));
				stmt.Parameters.Add(this.dbConn.getParameter("@DATE_TO",obj.PrDateTo));
				stmt.Parameters.Add(this.dbConn.getParameter("@TRAINING_COURSE_CODE",obj.PrTrainingCourseCode));

			if ( obj.isNew ) {
			} else {
			//only add primary key if we are updating and as the last parameter
								stmt.Parameters.Add(this.dbConn.getParameter("@EMPLOYEE_TRAINING_HISTORY_ID",obj.PrEmployeeTrainingHistoryId));
		}
	...
	public override void saveParents(IModelObject mo ){
		...
		{
			return EmployeeTrainingHistoryFactory.Create();
		}
	...
	public sealed class EmployeeTrainingHistoryDataUtils {
		...
		public static void saveEmployeeTrainingHistory(DataRow dr, ref EmployeeTrainingHistory mo) {
			if (mo == null) {
				mo = EmployeeTrainingHistoryFactory.Create();
			}
		...
		public static EmployeeTrainingHistory loadFromDataRow(DataRow r) {
			...
			IModelObject mo = EmployeeTrainingHistoryFactory.Create();
			a.DataSource = r;
			a.load(mo);
