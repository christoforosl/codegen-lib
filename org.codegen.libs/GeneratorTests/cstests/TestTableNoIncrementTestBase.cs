...
///************************************************************
///</comments>
[TestClass()] public class TestTableNoIncrementTestBase {
...
#region "Additional test attributes"
    ...
    [TestMethod()] public void TestLoadAndSaveTestTableNoIncrement() {
		
		ModelContext.beginTrans();
		try {
			
			CsModelMappers.TestTableNoIncrementDBMapper pdb = new CsModelMappers.TestTableNoIncrementDBMapper();
			...
			if (count == 0) {
				Assert.Inconclusive("No TestTableNoIncrement in database, table is empty");
			} else {
				/**
				using (DataContext ctx = DBUtils.Current().dbContext()) {

					var query = ctx.ExecuteQuery<TestTableNoIncrement>(@"SELECT * FROM " + pdb.SelectFromObjectName ).Skip(1).Take(1);
				...
				**/
				...
				TestTableNoIncrement p = pdb.findByKey(pid);
				TestTableNoIncrement p2 = (TestTableNoIncrement)p.copy();
				...
				Assert.IsTrue(p.PrTestTableNoIncrementId==p2.PrTestTableNoIncrementId,"Expected Field TestTableNoIncrementId to be equal");
				Assert.IsTrue(p.PrTestTableNoIncrement==p2.PrTestTableNoIncrement,"Expected Field TestTableNoIncrement to be equal");
				...
				p = ModelContext.Current.loadModelObject< TestTableNoIncrement >(p.Id);
				...
				System.IO.FileInfo jf = new System.IO.FileInfo(".\\TestTableNoIncrement.json");
